...
public class ShaderUtil extends Util {
    ...
    public ShaderUtil(String fragmentShadersrc, boolean notUsed) {
        ...
        try {
            vertexShaderID = createShader(mc.getResourceManager().getResource(new ResourceLocation("aluminium/shader/vertex.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (IOException e) {
    ...
    private final String roundedVaryingRect = "#version 120\n" +
            ...
            "\n" +
            "    // 根据像素位置确定使用哪个角的半径\n" +
            "    if (pos.x < rectHalf.x && pos.y > rectHalf.y) {\n" +
            "        // 左下角 (Bottom-Left)\n" +
            "        radius = radiusBL;\n" +
            "    } else if (pos.x >= rectHalf.x && pos.y > rectHalf.y) {\n" +
            "        // 右下角 (Bottom-Right)\n" +
            "        radius = radiusBR;\n" +
            "    } else if (pos.x < rectHalf.x && pos.y <= rectHalf.y) {\n" +
            "        // 左上角 (Top-Left)\n" +
